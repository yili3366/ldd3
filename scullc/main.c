		...
		size_t count, loff_t pos)
{
	...
	iov = kzalloc(sizeof(struct iovec), GFP_KERNEL);
	if (iov == NULL)
		return -1;
	iov->iov_base = buf;
	iov->iov_len = count;
	...
	if (write)
		...
		result = scullc_write(iocb->ki_filp, (char *)(iov->iov_base),  1, &pos);
	else
		//result = scullc_read(iocb->ki_filp, buf, count, &pos);
		result = scullc_read(iocb->ki_filp, (char *)(iov->iov_base),  1, &pos);
	...
	stuff = kmalloc (sizeof (struct async_work), GFP_KERNEL);
	if (stuff == NULL)
...
//	return scullc_defer_op(0, iocb, buf, count, pos);
	...
	for (i = 0; i < niov; i++) {
		if ((ret = scullc_defer_op(0, iocb, iov->iov_base, iov->iov_len, pos)) < 0)
			return ret;
	}
	return (niov * iov->iov_len);
//	return scullc_defer_op(0, iocb, iov->iov_base, iov->iov_len, pos);
}
...
//	return scullc_defer_op(1, iocb, (char __user *) buf, count, pos);
	...
	for (i = 0; i < niov; i++) {
		if ((ret = scullc_defer_op(1, iocb, iov->iov_base, niov, pos)) < 0)
			return ret;
	}
	return (niov * iov->iov_len);
//	return scullc_defer_op(0, iocb, iov->iov_base, iov->iov_len, pos);
...
int scullc_init(void)
{
	 ...
	 */
	scullc_devices = kzalloc(scullc_devs*sizeof (struct scullc_dev), GFP_KERNEL);
	if (!scullc_devices) {
		result = -ENOMEM;
		goto fail_malloc;
	}
